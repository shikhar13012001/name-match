...
def _has_match(token: str, token_list: list) -> bool:
    """True if *token* has a plausible match anywhere in *token_list*.
    
    Checks: fuzzy similarity >= 0.6, initial match, Soundex, Metaphone, nicknames.
    Used by conflict detection so surname-first reorder doesn't trigger a false conflict.
    """
    for t in token_list:
...
def extract_features(payer: str, account: str) -> dict:
    ...
    if len(t1) >= 2 and len(t2) >= 2:
        # Search ALL of t2 (not just t2[:-1]) — handles surname-first reorder
        # e.g. DIEDHIOU OUMAR vs OUMAR DIEDHIOU: both tokens present -> no conflict
